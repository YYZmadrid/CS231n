...
def slice_assignment_practice(x):
  ...
  Returns: x
  """
    val = torch.zeros((4, 6), dtype=int)
    val[:2, :2] = torch.tensor([[0, 1], [0, 1]])
    val[:2, 2:] = torch.full((2, 4), 2, dtype=int)
    val[2:, :2] = torch.tensor([[3, 4], [3, 4]])
    val[2:, 2:4] = torch.tensor([[3, 4], [3, 4]])
    val[2:, 4:] = torch.full((2, 2), 5, dtype=int)
    x[:4, :6] = val
    return x
...
def zero_row_min(x):
  ...
  """
    y = torch.clone(x)
    _, index_y = torch.min(y, dim=1)
    index_x = range(y.shape[0])
    y[index_x, index_y] = 0
    return y
