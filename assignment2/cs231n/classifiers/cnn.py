...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss, d_softmax = softmax_loss(out3, y)
        loss += 0.5 * self.reg * (np.sum(W1 * W1) + np.sum(W2 * W2) + np.sum(W3 * W3))
        d_out3, grads["W3"], grads["b3"] = affine_backward(d_softmax, cache3)
        d_out2, grads["W2"], grads["b2"] = affine_relu_backward(d_out3, cache2)
        d_out1, grads["W1"], grads["b1"] = conv_relu_pool_backward(d_out2, cache1)
