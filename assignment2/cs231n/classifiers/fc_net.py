...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        output_layer, cache_output_layer = affine_forward(hidden_layer, W2, b2)
        scores = output_layer.copy()
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        ###########################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        loss, d_output_layer = softmax_loss(output_layer, y)
        loss = loss + 0.5 * self.reg * np.sum(W1 * W1) + 0.5 * self.reg * np.sum(W2 * W2)
        d_hidden_layer, d_W2, d_b2 = affine_backward(d_output_layer, cache_output_layer)
        grads['W2'] = d_W2 + self.reg * W2
        grads['b2'] = d_b2
        _, d_W1, d_b1 = affine_relu_backward(d_hidden_layer, cache_hidden_layer)
...
class FullyConnectedNet(object):
    """
    A fullnlinearities, and a softmax loss function. This will also implement
    dropout and batch/layer normalization as options. For a network with L layers,
    the architecture will bey-connected neural network with an arbitrary number of hidden layers,
    ReLU no
    ...
    def loss(self, X, y=None):
        ...
        output_layer, output_layer_cache = affine_forward(hidden_layer, W, b)
        scores = output_layer.copy()
        # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ############################################################################
        #                             END OF YOUR CODE                             #
        ...
        loss, d_output_layer = softmax_loss(output_layer, y)
        d_hidden_layer, d_W, d_b = affine_backward(d_output_layer, output_layer_cache)
