...
def affine_backward(dout, cache):
    ...
    gradient = x.T @ dout
    dw = gradient[:x.shape[1]-1]
    db = gradient[x.shape[1]-1]
    dx = np.reshape(dout @ w.T, x_shape)
    # print(dx.shape)
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        sample_var = np.sum((x-sample_mean) ** 2, axis=0) / batch_size
        x_normalized = (x - sample_mean) / np.sqrt(sample_var + eps)
        x_shifted = gamma * x_normalized + beta
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        cache = (x, gamma, eps, sample_mean, sample_var, x_normalized)
        out = x_shifted
        ...
        cache = (x, gamma, eps, running_mean, running_var, x_normalized)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    x, gamma, eps, mean, var, x_normalized = cache
    batch_size = x.shape[0]
    dgamma = np.sum(x_normalized * dout, axis=0)
    dbeta = np.sum(dout, axis=0)
    dx_normalized = dout * gamma
    d_var = np.sum(dx_normalized * (x - mean) * -0.5 * ((var + eps) ** -1.5), axis=0)
    d_mean = np.sum(-dx_normalized / np.sqrt(var + eps), axis=0) + d_var * np.sum((-2 * (x - mean)), axis= 0) / batch_size
    dx = dx_normalized / np.sqrt(var + eps) + d_var * 2 * (x - mean) / batch_size + d_mean / batch_size
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_backward_alt(dout, cache):
    ...
    dgamma = np.sum(x_normalized * dout, axis=0)
    dbeta = np.sum(dout, axis=0)
    dx = gamma * (x.shape[0] * dout - dbeta - dgamma * x_normalized) / (x.shape[0] * np.sqrt(var + eps))
...
def layernorm_forward(x, gamma, beta, ln_param):
    ...
    sample_mean = np.vstack(sample_mean)
    sample_var = np.sum((x - sample_mean) ** 2, axis=1) / feature_size
    sample_var = np.vstack(sample_var)
    x_normalized = (x - sample_mean) / np.sqrt(sample_var + eps)
    cache = (x, gamma, eps, sample_mean, sample_var, x_normalized)
    out = gamma * x_normalized + beta
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
...
def layernorm_backward(dout, cache):
    ...
    d_mean = np.sum(-dx_normalized / np.sqrt(var + eps), axis=1) + d_var * np.sum((-2 * (x - mean)), axis=1) / feature_size
    d_var = np.vstack(d_var)
    d_mean = np.vstack(d_mean)
    dx = dx_normalized / np.sqrt(var + eps) + d_var * 2 * (x - mean) / feature_size + d_mean / feature_size
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        #######################################################################
        # TODO: Implement testing phase forward pass for inverted dropout.   #
        #######################################################################
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        ...
        for f in range(F):
            for i in range(new_H):
                for j in range(new_W):
                    for c in range(C):
                        field = xx[n, c, i * stride:i * stride + HH, j * stride:j * stride + WW]
                        kernel = w[f, c]
                        out[n, f, i, j] += np.sum(field * kernel)
                    out[n, f, i, j] += b[f]
            ...
            for i in range(new_H):
                for j in range(new_W):
                    for c in range(C):
                        field = xx[n, c, i * stride:i * stride + HH, j * stride:j * stride + WW]
                        kernel = w[f, c]
                        dw[f, c] += dout[n, f, i, j] * field
                        dxx[n, c, i * stride:i * stride + HH, j * stride:j * stride + WW] += dout[n, f, i, j] * kernel
                    db[f] += dout[n, f, i, j]
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(new_H):
                for j in range(new_W):
                    field = x[n, c, i * stride:i * stride + pool_height, j * stride:j * stride + pool_width]
                    max_index = np.where(field == np.max(field))
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****           #
    n, c, h, w = x.shape
    x_feature = np.transpose(x, axes=(0, 2, 3, 1))
    x_flatten = x_feature.reshape(n * h * w, c)
    out_flatten, cache = batchnorm_forward(x_flatten, gamma, beta, bn_param)
    out_feature = out_flatten.reshape(n, h, w, c)
    out = np.transpose(out_feature, axes=(0, 3, 1, 2))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    n, c, h, w = dout.shape
    dout_feature = np.transpose(dout, axes=(0, 2, 3, 1)).reshape(n * h * w, c)
    dx_flatten, dgamma, dbeta = batchnorm_backward_alt(dout_feature, cache)
    dx_feature = dx_flatten.reshape(n, h, w, c)
    dx = np.transpose(dx_feature, axes=(0, 3, 1, 2))
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
