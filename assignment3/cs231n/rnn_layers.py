...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    pre_h = h0
    for t in range(T):
        next_h, cache_t = rnn_step_forward(x[:, t, :], pre_h, Wx, Wh, b)
        h[:, t, :] = next_h
...
def rnn_backward(dh, cache):
    ...
    for t in range(T)[::-1]:
        dnext_h = dpre_h + dh[:, t, :]
        cache_t = cache[t]
        dx[:, t, :], dpre_h, dWx_t, dWh_t, db_t = rnn_step_backward(dnext_h, cache_t)
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    dh0 = dpre_h
...
def lstm_backward(dh, cache):
    ...
    for t in range(T)[::-1]:
        dpre_h = dpre_h + dh[:, t, :]
        cache_t = cache[t]
        dx[:, t, :], dpre_h, dpre_c, dWx_t, dWh_t, db_t = lstm_step_backward(dpre_h, dpre_c, cache_t)
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
    dh0 = dpre_h
