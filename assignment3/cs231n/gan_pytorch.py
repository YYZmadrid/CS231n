...
def discriminator_loss(logits_real, logits_fake):
    ...
    real_labels = torch.ones(batch_size).type(dtype)
    fake_labels = torch.zeros(batch_size).type(dtype)
...
def generator_loss(logits_fake):
    ...
    fake_labels = torch.ones(batch_size).type(dtype)
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
        ...
        nn.LeakyReLU(0.01),
        nn.MaxPool2d((2, 2), 2),  # 12
        nn.Conv2d(32, 64, (5, 5)),  # 8
        nn.LeakyReLU(0.01),
        nn.MaxPool2d((2, 2), 2),  # 4
        nn.Flatten(),
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    model = nn.Sequential(
        ...
        nn.ReLU(),
        nn.BatchNorm2d(64),
        nn.ConvTranspose2d(in_channels=64, out_channels=1, kernel_size=(4, 4), stride=(2, 2), padding=(1, 1)),
        # 1 * 28 * 28
        nn.Tanh(),
        Flatten(),
    )
