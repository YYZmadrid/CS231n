...
def content_loss(content_weight, content_current, content_original):
    ...
    content_loss = torch.sum(torch.pow((content_current.view(C_l, H_l * W_l) -  content_original.view(C_l, H_l * W_l)), 2)) * content_weight
    return content_loss
...
def gram_matrix(features, normalize=True):
    ...
    features1 = features.view(N, C, H * W)
    features2 = features.view(N, C, H * W).permute(0, 2, 1)
    gram = torch.bmm(features1, features2)
    if normalize:
      gram /= (H * W * C)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
      layer = style_layers[i]
      style_loss += torch.sum(torch.pow(style_targets[i] - gram_matrix(feats[layer]), 2)) * style_weights[i]
    return style_loss
...
def tv_loss(img, tv_weight):
    ...
    loss_h = torch.sum(torch.pow((img[:, :, 1:, :] - img[:, :, :-1, :]), 2))
    loss_w = torch.sum(torch.pow((img[:, :, :, 1:] - img[:, :, :, :-1]), 2))
    loss = (loss_h + loss_w) * tv_weight
    return loss
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
